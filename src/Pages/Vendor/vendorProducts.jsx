      ...
      >
        ...
        >
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px' }}>
            <Typography variant="h4" sx={{ fontFamily: 'Rubik', fontWeight: 'bold', color: '#091242' }}>
              عرض المنتجات
            </Typography>
          </Box>
            ...
            {products.map((product) => (
              <Grid key={product._id} item xs={12} sm={6} md={4} lg={3}>
                <Box sx={{ width: '100%', marginBottom: '20px' }}>
                  <VendorProductCard product={product} onDelete={handleDelete} />
                </Box>
              </Grid>
            ))}
          </Grid>
      ...
      <Box sx={{ height: '200vh', position: 'relative' }}>
        <Grid container sx={{ height: '200vh' }}>
          <Grid item xs={6} sx={{ backgroundColor: '#091242' }}>
            <Box sx={{ backgroundColor: 'rgba(255, 255, 255, 0.08)', height: '100%' }}></Box>
          </Grid>
        </Grid>
      </Box>
      <Footer/>
